...
class DTUDataset(Dataset):
            ...
            with open(self.pair_list_file) as f:
                ...
                for view_idx in range(num_viewpoint):
                    ...
                    if mode == 'train':
                        ...
                        metas.append((scan, ref_view, src_views, 3))#测试时光照等级强制为3
    ...
    def __getitem__(self, idx):
        ...
        # ref_img_file = getImageFile(self.data_root, self.args.mode, scan, ref_view, 3)#自建数据集
        ...
        for i in range(self.args.nsrc):
            ...
            # src_img_file = getImageFile(self.data_root, self.args.mode, scan, src_views[i], 3)#自建数据集
            # src_img = read_img(src_img_file)
            # center_image函数对图像的像素值进行归一化操作
        ...
        sample = {}
        # np.moveaxis 将一个数组的轴移动到新的位置。其他轴保持原来的顺序。这里将通道数放在了最前面。
        sample["ref_img"] = np.moveaxis(np.array(ref_img), 2, 0)# 将H W C变成CHW
        sample["ref_img_aug"] = np.moveaxis(np.array(ref_img_aug), 2, 0)
        sample["src_imgs"] = np.moveaxis(np.array(src_imgs), 3, 1)# 将 nsrc H W C变成nsrc C H W
